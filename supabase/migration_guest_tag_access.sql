-- Guest access via NFC / share URL secret (?id=&k=) — no Sign in required to read or append.
-- Run in Supabase SQL Editor after schema.sql and migration_public_share_and_photos.sql (if used).

-- --- Garments: capability secret (program into HTTPS URL as query param k) ---
...
-- --- Journal entries: guest rows (no auth.users row) ---
...
-- --- Replace single owner policy with granular policies (guest rows have user_id null) ---
...
-- --- RPC: read garment + all entries when k matches (no is_public required) ---
...
-- --- RPC: append guest memory (text; photos via Storage path documented below) ---
...
-- --- Storage: anon may upload under guest/{garment_id}/... only if garment has a tag secret set.
-- Anyone who knows garment_id could still upload here; pairing with ?k= on the web is recommended.
-- Tighter option later: Edge Function that verifies k then returns a signed upload URL.
